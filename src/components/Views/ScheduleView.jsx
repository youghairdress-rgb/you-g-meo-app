...
import { Calendar, Plus, Trash2, Save, Image as ImageIcon, X } from 'lucide-react';
import { DAYS } from '../../lib/constants';

export function ScheduleView({ postingRules, setPostingRules, saveToCloud, driveFiles = [] }) {
    const [selectingRuleId, setSelectingRuleId] = useState(null);

    const addRule = () => {
        setPostingRules([...postingRules, { id: Date.now(), type: 'daily', time: '10:00', topic: '', target: 'general', fileId: null, imageUrl: null, thumbnailUrl: null }]);
    };
    ...
    const handleSelectImage = (ruleId, file) => {
        const url = file.webContentLink;
        const thumb = file.thumbnailLink;
        updateRule(ruleId, 'fileId', file.id);
        updateRule(ruleId, 'imageUrl', url);
        updateRule(ruleId, 'thumbnailUrl', thumb);
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm p-6 space-y-6 animate-in fade-in border border-gray-100 min-h-[400px] font-bold">
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-3xl max-h-[80vh] overflow-y-auto shadow-2xl">
                        <div className="flex justify-between items-center mb-4 sticky top-0 bg-white z-10 pb-2 border-b">
                            <h3 className="font-bold text-lg">自動投稿する画像を選択</h3>
                            <button onClick={() => setSelectingRuleId(null)} className="p-2 hover:bg-gray-100 rounded-full"><X /></button>
                        </div>
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
                            {driveFiles && driveFiles.length > 0 ? driveFiles.map(file => (
                                <div key={file.id} onClick={() => handleSelectImage(selectingRuleId, file)} className="cursor-pointer hover:opacity-75 relative aspect-square group">
                                    <img src={file.thumbnailLink} className="w-full h-full object-cover rounded-lg border shadow-sm group-hover:scale-105 transition-transform" />
                                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 rounded-lg transition-colors" />
                                </div>
                            )) : (
                                <p className="col-span-full text-center text-gray-500 py-8">画像が見つかりません。Googleドライブを確認してください。</p>
                            )}
                        </div>
                    </div>
                </div>
            ...
            <button onClick={() => saveToCloud(null, postingRules)} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-xl hover:bg-blue-700 transition-all font-bold"><Save size={18} /> 予約設定を保存</button>
        </div>
    );
}
