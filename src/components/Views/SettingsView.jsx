...
import { Key, Instagram, Save } from 'lucide-react';

export function SettingsView({ config, setConfig, saveToCloud }) {
    return (
            ...
            <div className="space-y-6 font-bold">
                ...
                <div className="space-y-2 p-4 bg-purple-50 rounded-2xl border border-purple-100 font-bold">
                    ...
                    <input
                        ...
                        className="w-full p-2 border rounded-lg bg-white font-mono text-xs outline-none font-normal"
                        placeholder="自動取得されます"
                    />
                </div>
                <div className="p-5 bg-gray-50 rounded-2xl border border-gray-200 space-y-4 font-bold">
                    <h3 className="text-sm font-bold text-gray-700 tracking-widest decoration-gray-300 underline underline-offset-4 uppercase">Business Profile Credentials</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-[10px] font-normal">
                    ...
                    <div className="space-y-3 pt-2 font-normal">
                        <input type="text" value={config.gClientId} onChange={e => setConfig({ ...config, gClientId: e.target.value })} className="w-full p-2.5 border rounded-lg bg-white outline-none font-mono text-[10px]" placeholder="Client ID" />
                        <input type="password" value={config.gClientSecret} onChange={e => setConfig({ ...config, gClientSecret: e.target.value })} className="w-full p-2.5 border rounded-lg bg-white outline-none font-mono text-[10px]" placeholder="Client Secret" />
                        <textarea value={config.gRefreshToken} onChange={e => setConfig({ ...config, gRefreshToken: e.target.value })} className="w-full p-2.5 border rounded-lg bg-white h-20 font-mono outline-none text-[10px] leading-tight" placeholder="Refresh Token" />
                    </div>
                </div>
                <button onClick={() => saveToCloud(config, null)} className="bg-black text-white px-8 py-4 rounded-2xl font-bold w-full flex items-center justify-center gap-2 hover:bg-gray-800 shadow-2xl transition-all uppercase tracking-widest"><Save size={20} /> 設定を保存して同期</button>
            </div>
        </div>
    );
}
